# TP-Link TL-WDR7651 Unauthorized System Reset Vulnerability

## Overview

This repository documents an unauthorized access vulnerability found in the TP-LINK TL-WDR7651 router firmware. The issue exists in the VxWorks-based Web management service and can be triggered through the `httpParser -> httpPostHandle -> httpProcDataSrv` request handling path.
...
| Function | Address |
| --- | --- |
| `httpProcDataSrv` | `0x40558F88` |
| `httpPostHandle` | `0x4055C00C` |
| `httpParser` | `0x4055C86C` |
| `httpDoAuthorize` | `0x40571E8C` |
| `systemReset` | `0x4057C624` |

The vulnerable branch in `httpProcDataSrv` was observed around `0x4055969C` during reverse analysis.
...
The full request sample is stored in `wdr7651/payload.txt`.
